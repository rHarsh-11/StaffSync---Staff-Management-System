...
const Detail = () => {
  ...
  const changeStatus = async (id, status) => {
    try{
      ...
      if(response.data.success){
        navigate('/admin-dashboard/leaves', { replace: true })
      }
    } catch(error) {
      if(error.response && !error.response.data.success){
        console.error(error.response.data.error)
          ...
          <div className='flex space-x-3  mb-5'>
            ...
            {leave.status == "Pending" ? (
              <div className='flex space-x-2'>
                <button className='px-2 py-1 bg-green-600 hover:bg-green-700 text-white rounded' onClick={() => changeStatus(leave._id, "Approved")}>Approve</button>
                <button className='px-2 py-1 bg-red-500 hover:bg-red-600 text-white rounded' onClick={() => changeStatus(leave._id, "Rejected")}>Reject</button>
              </div>
            ): 
